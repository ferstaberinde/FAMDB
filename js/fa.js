...
var images = [imageCoop, imageAdv, imageV, imageVanilla, imageSlots, imageARMA3,
    imageARMA2
];
...
var arma2maps = ["All", "Chernarus", "Utes", "Takistan", "Desert", "Zargabad",
...
function FillMapSelect(game) {
    ...
    for (var x = 0; x < arr.length; x++) {
        var value = arr[x];
        ul.append("<li><a href='#' onclick=HandleMap(this)>" + value +
            "</a></li>");
    }
    for (var x = 0; x < arma2maps.length; x++) {
        if (arma2maps[x] != "All") $("#mapEdit").append("<option>" +
            arma2maps[x] + "</option>");
    }
    for (var x = 0; x < arma3maps.length; x++) {
        if (arma3maps[x] != "All") $("#mapEdit").append("<option>" +
            arma3maps[x] + "</option>");
    }
}
...
function ShowRow(rowobject) {
    ...
    if (rowobject.get("brokenMsg") !== null && rowobject.get("isBroken") !==
        null && rowobject.get("isBroken") === true) $MissionBrokenMsg.html(
        rowobject.get("brokenMsg").replace(/\n/g, "<br />"));
    else $MissionBrokenMsg.html("");
    $LastPlayedText.text(lastPlayed);
    if (currentGame == ARMA3) $gameImage.attr("src", imageARMA3);
    else $gameImage.attr("src", imageARMA2);
    $iamgesDiv.empty();
    $iamgesDiv.append('<img src="' + imageSlots +
        '" class="margin5left" alt="some_text" width="16" height="16"><small>' +
        rowobject.get("missionPlayers") + '</small></img>');
    if (rowobject.get("Session") == "V+") {
        $iamgesDiv.append('<img src="' + imageV +
            '" class="margin5left" alt="some_text" width="16" height="16"></img>'
        );
    }
    else {
        $iamgesDiv.append('<img src="' + imageVanilla +
            '" class="margin5left" alt="some_text" width="16" height="16"></img>'
        );
    }
    if (rowobject.get("missionType") == "Co-op") {
        $iamgesDiv.append('<img src="' + imageCoop +
            '"  class="margin5left" alt="some_text" width="16" height="16"></img>'
        );
    }
    else {
        $iamgesDiv.append('<img src="' + imageAdv +
            '"  class="margin5left" alt="some_text" width="16" height="16"></img>'
        );
    }
    ...
    query.first({
        success: function(adminRole) {
            if ((Parse.User.current() !== null && Parse.User.current()
                    .id == rowobject.get("createdBy").id) ||
                adminRole !== null && Parse.User.current() !=
                null) {
    ...
    Parse.User.logIn(user, pass, {
        ...
        error: function(user, error) {
            ...
            if (error.code === 101) {
                ...
                str = "Whoops something went wrong! (" +
                    error.message + ")";
            }
            $("#loginPasswordError").append("<div id='" + id +
                "' class='alert smallnote center-block alert-warning alert-dismissable'><button type='button' class='close' onclick='Close(" +
                id +
            ...
            switch (error.code) {
                case Parse.Error.USERNAME_TAKEN:
                    var id = Math.floor((Math.random() * 100) +
                        1);
                    var str =
                        "<small>Username has already been taken</small>";
                    $("#signUserError").append("<div id='" + id +
                        "' class='alert smallnote center-block alert-danger alert-dismissable'><button type='button' class='close' onclick='Close(" +
                        id +
                        ")' aria-hidden='true'>&times;</button>" +
                        str + "</div>");
                    break;
                case -1:
                    if ($("#signUsername").val() === "") {
                        var id = Math.floor((Math.random() *
                            100) + 1);
                        var str =
                            "<small>You need a username</small>";
                        $("#signUserError").append("<div id='" +
                            id +
                            "' class='alert smallnote center-block alert-warning alert-dismissable'><button type='button' class='close' onclick='Close(" +
                            id +
                            ")' aria-hidden='true'>&times;</button>" +
                    ...
                    if ($("#signPassword").val() === "") {
                        var id = Math.floor((Math.random() *
                            100) + 1);
                        var str =
                            "<small>You need a password</small>";
                        $("#signPasswordError").append(
                            "<div id='" + id +
                            "' class='alert smallnote center-block alert-warning alert-dismissable'><button type='button' class='close' onclick='Close(" +
                            id +
                            ")' aria-hidden='true'>&times;</button>" +
                ...
                case Parse.Error.EMAIL_TAKEN:
                    var id = Math.floor((Math.random() * 100) +
                        1);
                    var str =
                        "<small>Email has already been registered</small>";
                    $("#signEmailError").append("<div id='" +
                        id +
                        "' class='alert smallnote center-block alert-danger alert-dismissable'><button type='button' class='close' onclick='Close(" +
                        id +
                        ")' aria-hidden='true'>&times;</button>" +
    ...
    query.find({
        success: function(results) {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                var text = "<tr id='" + object.id + "'><td>" +
                    object.get("missionName") + "</td><td>" +
                    object.get("missionPlayers") + "</td><td>" +
                    moment(object.get("lastPlayed")).fromNow() +
                    "</td></tr>";
                $("#missionTable tbody").append(text);
                if (object.get("isBroken") !== null && object.get(
                    "isBroken") === true) {
...
function TryEdit() {
    ...
    if (missionName === "") {
        ...
        $("#error-missionName").append("<div id='" + id +
            "' class='alert smallnote center-block alert-warning alert-dismissable'><button type='button' class='close' onclick='Close(" +
            id + ")' aria-hidden='true'>&times;</button>" + str +
            "</div>")
        error = true;
    }
    if (missionMap === "") {
    ...
    if (session == "") {
        ...
        $("#error-session").append("<div id='" + id +
            "' class='alert smallnote center-block alert-warning alert-dismissable'><button type='button' class='close' onclick='Close(" +
            id + ")' aria-hidden='true'>&times;</button>" + str +
            "</div>")
        error = true;
    ...
    if (missionType == "") {
        ...
        $("#error-missionType").append("<div id='" + id +
            "' class='alert smallnote center-block alert-warning alert-dismissable'><button type='button' class='close' onclick='Close(" +
            id + ")' aria-hidden='true'>&times;</button>" + str +
            "</div>")
        error = true;
    ...
    if (missionDesc == "") {
        ...
        $("#error-missionDesc").append("<div id='" + id +
            "' class='alert smallnote center-block alert-warning alert-dismissable'><button type='button' class='close' onclick='Close(" +
            id + ")' aria-hidden='true'>&times;</button>" + str +
            "</div>")
        error = true;
...
$MarkPlayedButton.click(function() {
    if (currentRow == "" || currentRow == null) return;
