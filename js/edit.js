      ...
      success: function(object) {
        ...
        query.find({
          success: function(roles) {
            ...
            {
              if(ACL.getWriteAccess(roles[x]) || ACL.getWriteAccess(currentUser))
              {
                window.row = object;
                ...
                $("#missionBrokenMessage").val(object.get("brokenMsg"));
...
$('#missionSave').click(function(){
  ...
  if(missionName == "" || missionName == null)
    {MissionSaveError("Name is invaild.");return false;}
  if(missionGame == "" || missionGame == null)
    {MissionSaveError("Invaild game selection");return false;}
  if(missionIsland == "" || missionIsland == null)
    {MissionSaveError("Invaild Island selection");return false;}
  if(missionSession == "" || missionSession == null)
    {MissionSaveError("Invaild Session selection");return false;}
  if(missionType == "" || missionType == null)
    {MissionSaveError("Invaild mission type selection");return false;}
  if(!(missionSlots > 0) || missionSlots == null)
    {MissionSaveError("Invaild data in Slots field");return false;}
  if(missionsAuthors == "" || missionsAuthors == null)
    {MissionSaveError("Invaild data in Author field");return false;}
  if(missionDescription == "" || missionDescription == null)
    {MissionSaveError("Invaild data in Mission Description field");return false;}

  if(isBroken)
  {
    if(missionBrokenMessage == "" || missionBrokenMessage == null)
    {MissionSaveError("Missing Broken message");return false;}
  }
